...
const NOTES = [
  {
    ...
    items: [
      ...
      { title: "PCA (extended)", path: "PCA/pca27.pdf", lab: "#/pca" },
      { title: "SVM & ensembles", path: "Ensemble techiques/AIML27SVMandEnsemble_beautified.pdf", lab: null },
      { title: "Generalization", path: "Generalization/ML_Morning_Session_Notes.pdf", lab: null },
  ...
  },
  {
    ...
    items: [
      { title: "Linear regression", path: "reading-materials/U1/2026-05-16_Intro-to-ML-Algorithms-Key-Python-Tools/lecture-notes/Linear-Regression__Linear_regression_V1.8.pdf", lab: "#/linreg" },
...
export function renderNotes(root) {
  root.innerHTML = `
    <section class="hero narrow">
      ...
      <h1>Notes &amp; reading</h1>
      <p class="lede">
        Study PDFs hosted on
        <a class="ext" href="${NOTES_REPO}" target="_blank" rel="noreferrer">GitHub</a>.
        Open a document, or jump into the matching interactive lab when one exists.
      </p>
    </section>
    ${NOTES.map(
      (sec) => `
